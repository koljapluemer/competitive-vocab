<template>
  <div class="text-center">
    <h2 class="text-4xl my-10 font-bold">{{ currentQuestion.word_native || "Loading..." }}</h2>
...
const selectAnswer = async (selected) => {
  if (selected === currentQuestion.value.word_target) {
    ...
    await vocabStore.registerRepetition(currentQuestion.word_native, Math.max(0, 4 - numberOfWrongClicks.value), 4);
